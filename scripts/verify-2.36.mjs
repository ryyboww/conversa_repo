...
add('release version 2.36.0', pkg.version === '2.36.0');
...
add('homepage portrait has softened corners', /home-hero__portrait[\s\S]*border-radius:\s*18px/.test(homeCss));
add('homepage portrait stays static with no scroll animation', !/home-hero__portrait[\s\S]*position:\s*sticky/.test(homeCss));
add('inquiry grid uses three equal cells', /home-inquiry-grid[\s\S]*grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/.test(homeCss));
add('inquiry grid has no inter-cell gap', /home-inquiry-grid[\s\S]*gap:\s*0/.test(homeCss));
add('inquiry cells use equal internal padding', /home-inquiry-item[\s\S]*padding:\s*14px\s+clamp\(16px,\s*2vw,\s*28px\)/.test(homeCss));
add('inquiry cells use subtle dividers', /home-inquiry-item \+ \.home-inquiry-item[\s\S]*border-left/.test(homeCss));

let failed=0;
for (const [name,ok] of checks) { console.log(`${ok?'PASS':'FAIL'}  ${name}`); if(!ok) failed++; }
console.log(`\n${checks.length-failed}/${checks.length} 2.36 checks passed.`);
